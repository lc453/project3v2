...
export default {
  ...
  methods: {
    getAdventurers() {
      try {
        let response = this.$root.$data.adventurers;
        console.log(this.sortby);
        if (this.sortby==='') {
          this.adventurers = response;
        }
        if (this.sortby==='Strength') {
          this.adventurers = response.sort((a, b) => a.strength < b.strength);
        }
        if (this.sortby==='Agility') {
          this.adventurers = response.sort((a, b) => a.agility < b.agility);
        }
        if (this.sortby==='Durability') {
          this.adventurers = response.sort((a, b) => a.durability < b.durability);
        }
        if (this.sortby==='Intelligence') {
          this.adventurers = response.sort((a, b) => a.intelligence < b.intelligence);
        }
        if (this.sortby==='Cost') {
          this.adventurers = response.sort((a, b) => a.cost > b.cost);
        }
        return true;
      } catch (error) {
        console.log(error);
